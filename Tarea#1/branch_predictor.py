...
# Esto permite correr el programa de forma más intuitiva
# mediante el uso de argumentos.
# Ustedes pueden agregar más opciones en caso de que
# sean necesarias
...
parser.add_option(
        "--bp",
        dest="branch_predictor_type"
        )
parser.add_option(
        "-g",
        dest="global_history_size"
        )
parser.add_option(
        "-t",
        dest="TRACE_FILE",
...
# En cada caso instanciamos el predictor y
# luego usamos la función print_info para
# verificar que se esté utilizando correctamente
...
# Si --bp 1 entonces usamos g-shared
    ...
    branch_predictor = gshared(
            int(options.bits_to_index),
            int(options.global_history_size)
            )
    branch_predictor.print_info()
...
with gzip.open(options.TRACE_FILE, 'rt') as trace_fh:
    ...
    for line in trace_fh:
        ...
        # Quitamos espacios extra al final y extraemos el
        # PC y el resultado del salto

        line = line.rstrip()
        PC, result = line.split(" ")

        # Todos los predictores deben tener 2 funciones
        # 1. prediction: que con el estado actual del
        # predictor y el PC del salto predicen si el
        # salto se tomará, o no

        prediction = branch_predictor.predict(PC)

        # 2. UPDATE: Con el estado actual del predictor,
        # el PC del salto y el resultado real
        # de la predicción actualizamos el estado
        # del predictor para próximas predicciones

        branch_predictor.update(PC, result, prediction)

        # NOTA:  el update DEBE HACERSE DESPUÉS del predict,
        # pues en la realidad, el resultado del branch se
        # obtendrá varios ciclos después de hacer la predicción

        # Este código de abajo es para sólo hacer las primeras
        # 25 líneas del programa, de forma que sea más
        # fácil ir revisando el progreso que van haciendo
        # i+=1           #DEBUG
        # if i == 25:    #DEBUG
        #    break      #DEBUG

# Una vez finalizado el archivo, se imprimen las estadísticas
# de la corrida
