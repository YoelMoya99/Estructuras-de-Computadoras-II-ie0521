class gshared:
    def __init__(self, bits_to_index, global_history_size):
        ...
        if self.global_history_size > self.bits_to_index:
            print(
                "El tamaño del registro de historia es mayor que"
                " los bits para indexar la tabla se limitará el "
                "registro a " + str(self.bits_to_index) + "bits"
                )
    ...
    def print_info(self):
        ...
        print(
            "\tEntradas en el Predictor:\t\t\t" +
            str(2**self.bits_to_index)
            )
        print(
            "\tTamaño de los registros de historia global:\t" +
            str(self.global_history_size)
            )
    ...
    def update(self, PC, result, prediction):
        ...
        if result == "T":
            self.global_history_reg = (
                    self.global_history_reg[-self.global_history_size+1:] +
                    "1"
                    )
        else:
            self.global_history_reg = (
